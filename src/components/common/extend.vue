<!--
 * @Author: Mr.wang
 * @Date: 2021-04-10 21:19:07
 * @LastEditTime: 2021-05-06 16:39:28
 * @Description: 
 * @LastEditors: Mr.wang
...
<template>
  ...
  >
    <div class="oe-flex extend-box">
      <div
        class="choice-box"
        :class="[paymentConfigure.capacity_item_id == item.capacity_item_id ? 'currentStyle' : '']"
        @click="currentTab(item)"
        v-for="(item, index) in mealData.items"
        :key="Number(index)"
      >
  ...
  <el-dialog title="空间扩展" v-model="purchaseShow" width="719px">
    <div class="purchase-box">
      ...
      <el-form class="form-info" label-width="250px">
        <el-form-item label="实付金额"
          ><span class="purchase-price">¥{{ orderInfo.price }}</span></el-form-item
        >
        <el-form-item label="订单编号">{{ orderInfo.sn }}</el-form-item>
        <el-form-item label="支付方式">{{ orderInfo.pay_name }}</el-form-item>
        <el-form-item label="套餐名称">{{ orderInfo.capacity_name }}</el-form-item>
      </el-form>
...
<script setup lang="ts">
  ...
  watch(props, () => {
    if (props.show) {
      paymentConfigure.value.capacity_item_id = props.mealData.items[0].capacity_item_id
      paymentConfigure.value.capacity_id = props.mealData.capacity_id
      paymentConfigure.value.price = props.mealData.items[0].price
    }
  })
</script>
