 ...
 * @LastEditTime: 2021-05-10 15:16:20
 * @Description: 商品分组
 * @LastEditors: Mr.wang
-->
<template>
  <el-card shadow="never" class="custom-crad-container">
    <div class="header">
      <div class="header-left">
        <el-button class="add-goods" type="primary" size="small" @click="jumpAddGrouping()">
    ...
    >
      <template v-slot:empty>
        <el-empty class="oe-m-t-30" description="暂无商品分组" />
      </template>
      <el-table-column type="selection" width="55"> </el-table-column>
      <el-table-column label="分组名称" prop="name"> </el-table-column>
      <el-table-column label="描述" prop="summarize">
        <template #default="scoped">
          <div class="overflow-ellipsis">{{ scoped.row.summarize }}</div>
        </template>
      </el-table-column>
...
<script lang="ts" setup>
  ...
  const selectAllChange = () => {
    ElMessageBox.confirm('此操作将永久删除选中分组, 是否继续?', '删除分组', {
      ...
      await reqGoodsGroupDel({ id: arr.join() })
      ElMessage({
        type: 'success',
        message: '删除成功!'
      })
  ...
  const delCommodity = (id: string) => {
    ElMessageBox.confirm('此操作将永久删除该分组, 是否继续?', '删除分组', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      await reqGoodsGroupDel({ id })
      ElMessage({
        type: 'success',
        message: '删除成功!'
      })
