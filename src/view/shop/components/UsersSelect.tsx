...
const SetMealInstance = defineComponent({
  ...
  setup: (props, context) => {
    ...
    // 分页逻辑
    const {pageSize, total, currentPage, list } = useListPagination({
      getList: async ({pageSize, total, currentPage }) => {
        const { data } = await reqUserList({ page: currentPage.value,search:searchText.value,limit:16 })
        pageSize.value = 16
    ...
    // 点击选择项
    return () => <CalMessageDialog width={1200} title="选择用户" v-model={showModal.value} {...{ onOk: onEmitConfirm }}>
      {/* 顶部输入框 */}
      <div class="mb-28 flex justify-end">
        <CalInput placeholder="请输入会员昵称" class="w-160" v-model={searchText.value} />
      </div>
      {/* 沾粘布局容器 */}
 ...
 * @param props 组件参数
 */
const UserSelect = (props: SetMealProps = {}) => {
