 ...
 * @LastEditTime: 2021-05-10 15:19:21
 * @Description: 登录页面
...
<template>
  <div class="login-container-right-panel">
    ...
    <el-tabs class="custom-tabs" v-model="currentTab">
      <el-tab-pane label="账号登录" name="account">
        <el-form :model="accountInfo" class="login-container-form">
          <el-form-item>
            <el-input placeholder="账号/邮箱/手机号" v-model="accountInfo.account"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input placeholder="密码" type="password" v-model="accountInfo.password"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input placeholder="验证码" v-model="accountInfo.captcha" @keyup.enter="onLogin">
      ...
      <el-tab-pane label="手机号登录" name="phone">
        <el-form :model="mobileInfo" class="login-container-form">
          <el-form-item>
            <el-input placeholder="手机号" v-model="mobileInfo.mobile"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input placeholder="验证码" v-model="mobileInfo.code" @keyup.enter="onLogin">
              <template v-slot:append>
    ...
    <!-- <el-button class="login-container-subs-btns wechat-login" type="primary">
      <span class="text"><i class="ad-weChat icon"></i>微信登录</span>
    </el-button> -->
  </div>
</template>
<script lang="ts" setup>
  ...
  import { reqAuthSendLoginCode, reqCaptcha } from '@/api/user'
  ...
  const sendEmailCode = () => {
    ...
    count.value = 60
    resetTime(count)
    reqAuthSendLoginCode({ mobile: mobileInfo.mobile }).then((res) => {
      // console.log(res)
    })
  }
  // 获取账号登录验证码图片
  ...
  const onLogin = async () => {
    if (currentTab.value === 'account') {
      ...
      await store.dispatch('mobileLogin', mobileInfo)
    }
  }
  // 跳转忘记密码页面
  const jumpForget = () => {
