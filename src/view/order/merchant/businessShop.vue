 ...
 * @LastEditTime: 2021-07-09 11:55:55
 * @Description: 营业店铺
 * @LastEditors: Mr.wang
  ...
  <a-form class="-ml-40 flex flex-wrap">
    ...
    <a-form-item>
      <cal-input-select placeholder="支付方式" v-model="form.pay_code" class="w-192 mx-40">
        <a-select-option :value="''">全部</a-select-option>
        <a-select-option v-for="(item, i) in paymentMode" :key="Number(i)" :value="item.key">{{
          item.name
        }}</a-select-option>
      </cal-input-select>
    </a-form-item>
    ...
    <cal-table class="mt-28" contentAlign="left" :list="list" sync-space>
      ...
      <cal-table-option title="店铺名称/ID" min-width="158" v-slot="{ item }">
        <div class="flex items-center justify-items-center">
          <div class="w-48 h-48 rounded-full mr-10">
            <cal-image :fallback="shopLogo" :src="item.logo_path" />
          </div>
          <div class="flex flex-col">
            <div class="mb-4. text-primary">{{ item.shop_name }}</div>
            <div class="text-primary">ID:{{ item.shop_id }}</div>
          </div>
        </div>
      </cal-table-option>
      <cal-table-option title="联系人/联系方式" min-width="104" v-slot="{ item }">
        <div class="flex flex-col text-primary">
          <div class="mb-6">{{ item.liaisons }}</div>
          <div>{{ item.liaisons_tel }}</div>
        </div>
      </cal-table-option>
      ...
      <cal-table-option
        ...
        sort
        v-slot="{ content }"
        >{{ $formatUnix(content) }}</cal-table-option
      >
      <cal-table-option title="操作" min-width="186" border-left v-slot="{ item }">
        <div>
          <cal-button
            ...
            style="width: 76px"
            class="mr-10 my-6"
            @click="$router.push('/order/merchant/orderDetail?type=business&id=' + item.id)"
            >查看</cal-button
          >
          <cal-button
            size="small"
...
<script lang="ts" setup>
  ...
  const { list, total, pageSize, currentPage, resetList } = useListPagination({
    async getList({ total, pageSize, currentPage }) {
      const { data } = await reqBusinessOrderList({
        search: {
          ...form,
  ...
  const onExport = async () => {
    const { data } = await reqBusinessExport({
      limit: pageSize.value,
      search: {
        ...form,
        status: ['', 0, 2, -1][activeKey.value],
    ...
    exportReport(data, '营业店铺订单' + Date.now())
  }
</script>
