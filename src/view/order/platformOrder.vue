<!--
 * @Author: Zhilong
 * @Date: 2021-05-27 09:52:46
 * @LastEditTime: 2021-07-08 11:38:30
 * @Description: 平台订单
 * @LastEditors: Zhilong
 * @autograph: ⚠ warning!  ⚠ warning!  ⚠ warning!   ⚠野生的页面出现了!!
-->

<template>
  <a-form class="-ml-40 flex flex-wrap">
    <a-form-item>
      <cal-input placeholder="订单编号/产品名称" v-model="form.osid" class="w-192 mx-40" />
    </a-form-item>
    <a-form-item>
      <cal-input-select placeholder="订单类型" v-model="form.type" class="w-192 mx-40">
        <a-select-option :value="''">全部</a-select-option>
        <a-select-option :value="1">应用订单</a-select-option>
        <a-select-option :value="6">店铺订单</a-select-option>
        <a-select-option :value="3">渠道订单</a-select-option>
        <a-select-option :value="2">模板订单</a-select-option>
        <a-select-option :value="4">主题订单</a-select-option>
        <a-select-option :value="5">空间扩容订单</a-select-option>
      </cal-input-select>
    </a-form-item>
    <!-- <a-form-item>
      <cal-input-select placeholder="订单状态" v-model="form.status" class="w-192 mx-40">
        <a-select-option :value="''">全部</a-select-option>
      </cal-input-select>
    </a-form-item> -->
    <a-form-item>
      <cal-input-select placeholder="支付方式" v-model="form.pay_code" class="w-192 mx-40">
        <a-select-option :value="''">全部</a-select-option>
        <a-select-option v-for="(item, i) in paymentMode" :key="i" :value="item.key">
          {{ item.name }}
        </a-select-option>
      </cal-input-select>
    </a-form-item>
    <!-- <a-form-item>
      <cal-input-select placeholder="操作人" v-model="form.payType" class="w-192 mx-40">
        <a-select-option value="jack">Jack</a-select-option>
      </cal-input-select>
    </a-form-item> -->
    <a-form-item>
      <cal-input-date placeholder="下单时间" v-model="form.createtime" class="w-192 mx-40" />
    </a-form-item>
    <a-form-item>
      <cal-input-date placeholder="完成时间" v-model="form.finshtime" class="w-192 mx-40" />
    </a-form-item>
    <a-form-item>
      <cal-input-date placeholder="关闭时间" v-model="form.canceltime" class="w-192 mx-40" />
    </a-form-item>
  </a-form>
  <div class="mb-28">
    <cal-button type="primary" class="w-112 mr-28" @click="resetList()">筛选</cal-button>
    <cal-button class="w-112" @click="resetFields(), resetList()">重置条件</cal-button>
  </div>
  <cal-tabs v-model:activeKey="activeKey" @change="resetList()">
    <cal-tab-pane v-for="(item, i) in tabs" :key="i" hide>
      <template #tab>
        <span>{{ item.title }}</span>
        <cal-theme-icon type="numberTag"  :state="i === activeKey" v-if='i !== 0'>
          {{ item.count || 0 }}
        </cal-theme-icon>
      </template>
    </cal-tab-pane>
  </cal-tabs>
  <!-- table固定内容 -->
  <cal-tab-pane alone :min-height="600">
    <div class="flex justify-between">
      <cal-button class="w-144" type="primary" @click="onExport">导出</cal-button>


      <cal-page-select v-model="pageSize" />
    </div>
    <cal-table class="mt-28" contentAlign="left" :list="list" sync-space>
      <cal-table-option title="订单编号" index="sn" min-width="182" />
      <cal-table-option title="订单类型" index="type" min-width="86" sort v-slot="{ content }">
        {{
          ['其他订单', '应用订单', '模板订单', '渠道订单', '主题订单', '控件扩容订单', '店铺订单'][
            content
          ] || '其他订单'
        }}
      </cal-table-option>
      <cal-table-option title="订单金额" index="amount" min-width="66" />
      <cal-table-option title="支付方式" index="pay_name" min-width="80" />
      <!-- <cal-table-option title="站点编号" min-width="66"> 123456 </cal-table-option> -->
      <!-- <cal-table-option title="操作人" min-width="80"> 超级管理员 </cal-table-option> -->
      <!--  <cal-table-option title="联系人/联系方式" min-width="104">
        <div class="flex flex-col">
          <a href="javascript:;" class="mb-6">我是店铺名称</a>
          <a href="javascript:;">ID：6464641</a>
        </div>
      </cal-table-option> -->
      <cal-table-option title="订单状态" min-width="56" index="status" v-slot="{ content }">
        {{ statusMode[content] }}
      </cal-table-option>
      <cal-table-option
        title="下单时间"
        min-width="145"
        index="createtime"
        sort
        v-slot="{ content }"
      >
        {{ $formatUnix(content) }}
      </cal-table-option>
      <cal-table-option title="操作" min-width="186" border-left v-slot="{ item }">
        <div>
          <cal-button
            size="small"
            style="width: 76px"
            class="mr-10 my-6"
            @click="$router.push('/order/orderDetail?id=' + item.id)"
          >
            查看
          </cal-button>
          <cal-button
            size="small"
            style="width: 76px"
            class="mr-10 my-6"
            v-if="item?.status === 0"
            @click="onPurchaseItem(item)"
          >
            去支付
          </cal-button>
          <cal-button
            size="small"
            style="width: 76px"
            class="mr-10 my-6"
            v-if="item?.status === 0"
            @click="cancelOrder(item.id, item)"
          >
            取消
          </cal-button>
        </div>
      </cal-table-option>
    </cal-table>
    <template #sticky>
      <div class="flex justify-center">
        <cal-pagination v-model="currentPage" v-model:pageSize="pageSize" :total="total" />
      </div>
    </template>
  </cal-tab-pane>
</template>
<script lang="ts" setup>
  import { reactive, ref } from 'vue-demi'
  import { useForm } from '@ant-design-vue/use'
  import { useListPagination } from '@/hooks/use-listPagination'
  import {
    reqPlatformOrderList,
    reqPlatformPaymentMode,
    reqCancelPlatformOrder,
    reqPlatformExport
  } from '@/api/order'
  import { CalMessageDialog } from '~/CustomAntds/CalMessageDialog'
  import { message } from 'ant-design-vue'
  import { CalPaymentOrder } from './merchant/components/MessageDialogTemplate'
  import { exportReport } from '@/utils/vue-utils'
  import { CalPaymentMessage } from '@/components/Business/CalPaymentMessage'

  /** 筛选列表 */
  const form = reactive({
    osid: '',
    type: '',
    // status: '',
    pay_code: '',
    createtime: '',
    finshtime: '',
    canceltime: ''
  })

  /** 交易状态枚举 */
  enum statusEnum {
    交易关闭 = -2,
    已关闭,
    待付款,
    已完成
  }
  const statusMode = statusEnum

  /* 取消订单 */
  const cancelOrder = async (id: number, item: any) => {
    await CalMessageDialog.warning({
      title: '取消订单',
      content: '您确定取消该订单么？'
    })
    await reqCancelPlatformOrder({ id })
    item.status = -1
    await message.success('取消订单成功', 1.5)
  }

  /* 去支付按钮 */
  const toPayment = async () => {
    await CalPaymentOrder()
  }

  /* 支付方式选项 */
  const paymentMode = ref<any>([])
  ;(async () => {
    const { data } = await reqPlatformPaymentMode()
    paymentMode.value = data.data
  })()

  /** 重置筛选 */
  const { resetFields } = useForm(form, {})

  /** 当前的tabs索引 */
  const activeKey = ref(0)

  //列表渲染
  const tabs = ref([
    {
      title: '全部订单',
      count: 0
    },
    {
      title: '待付款',
      count: 0
    },
    {
      title: '已完成',
      count: 0
    },
    {
      title: '已关闭',
      count: 0
    }
  ])

  const onPurchaseItem = async (item: any) => {
    // 调起支付
    await CalPaymentMessage({
      title: '支付订单',
      price: item.amount,
      type: 'renew',
      params: {
        sn: item.sn,
        type: item.type,
        // system_id: 1,
        code: item.pay_code,
        associate_id: item.id,
        buy_type: item.buy_type
      }
    })
    await resetList()
    message.success('支付成功')
  }

  /** 获取数据列表 */
  const { list, total, pageSize, currentPage, resetList } = useListPagination({
    async getList({ total, pageSize, currentPage }) {
      const { data } = await reqPlatformOrderList({
        search: {
          ...form,
          status: ['', 0, 1, -1][activeKey.value],
          createtime: form.createtime && [form.createtime, form.createtime + 60 * 60 * 24],
          finshtime: form.finshtime && [form.finshtime, form.finshtime + 60 * 60 * 24],
          canceltime: form.canceltime && [form.canceltime, form.canceltime + 60 * 60 * 24]
        },
        page: currentPage.value,
        limit: pageSize.value
      })
      total.value = data.data.total
      /* 全部订单才赋值 */
      if (activeKey.value === 0) {
        tabs.value[0].count = data.data.total
        tabs.value[1].count = data.data.no_payment
        tabs.value[2].count = data.data.completed
        tabs.value[3].count = data.data.cancel_order
      }
      return data.data.data
    }
  })

  /** 导出列表 */
  const onExport = async () => {

    const { data } = await reqPlatformExport({
      limit: pageSize.value,
      search: {
        ...form,
        status: ['', 0, 1, -1][activeKey.value],
        createtime: form.createtime && [form.createtime, form.createtime + 60 * 60 * 24],
        finshtime: form.finshtime && [form.finshtime, form.finshtime + 60 * 60 * 24],
        canceltime: form.canceltime && [form.canceltime, form.canceltime + 60 * 60 * 24]
      }
    })
    exportReport(data, '平台订单' + Date.now())
  }
</script>
