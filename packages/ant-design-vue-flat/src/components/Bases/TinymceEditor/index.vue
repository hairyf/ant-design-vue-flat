 ...
 * @LastEditTime: 2021-07-05 16:39:27
 * @Description: 
 * @LastEditors: Mr.wang
-->
<template>
  <div v-if="disabled" :style="style" v-html="content" />
  <editor
    v-else
...
<script lang="ts" setup>
  ...
  import tinymce, * as t from 'tinymce'
  import Editor from '@tinymce/tinymce-vue'
  import imageSelector from './imageSelector'
  import tinymceZhCNUrl from '@/assets/tinymce-langs/zh_CN.js?url'
  import './tinymceImports'
  import { analyUnit } from '@/utils'
  const emit = defineEmit()
  /** 添加自定义图片选择 */
  tinymce.PluginManager.add('imageSelector', imageSelector)
  /** 默认配置 */
  ...
  const editorOptions = reactive<t.RawEditorSettings>({
    skin: false,
    content_css: false,
    width: props.width,
    height: props.height,
    language_url: tinymceZhCNUrl,
    language: 'zh_CN',
    // 静态资源基本路径
    // base_url: sliceBaseUrl('@/assets/tinymce-public/'),
    // 插件列表
    plugins: [
      // 高级列表 描点 自动链接 自动存稿 源码编辑 代码示例 颜色选择 右键菜单 文字方向 emoji标签 全屏编辑 分割线 图片
      'imageSelector advlist anchor autolink autosave code codesample colorpicker contextmenu directionality fullscreen hr',
    ...
    toolbar: [
      ...
      'hr bullist numlist link charmap preview anchor pagebreak insertdatetime table forecolor backcolor fullscreen imageSelector'
      // 媒体
      // 'media image bdmap'
    ],
