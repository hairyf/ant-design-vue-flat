 ...
 * @LastEditTime: 2021-07-07 17:51:10
 * @Description: 设置渠道
  ...
  >
    <cal-tabs v-model:activeKey="activeKey" class="w-804 mx-auto">
      <cal-tab-pane tab="基础设置" :key="1" :minHeight="400">
        ...
        <a-form-item label="渠道图标" name="is_icon">
        ...
        <a-form-item label="渠道简介" name="remark" class="remark-introduction">
          <cal-textarea
      ...
      <cal-tab-pane tab="规格设置" :key="2" :minHeight="400" showCount :maxlength="50">
        ...
        <a-form-item label="售卖价格" name="sale_spec">
...
<script lang="ts" setup>
  ...
  import { reqChannelRead, reqChannelEdit } from '@/api/channel'
  ...
  reqChannelRead({ id: route.query.id }).then((res) => {
  ...
  const { rules, validate } = useAntdForm(channelFormRef, {
    name: [{ required: true, message: '请输入渠道名称', trigger: 'blur' }],
    is_icon: [{ required: true, message: '请选择渠道图标', trigger: 'blur', type: 'number' }],
  ...
  const submitForm = async () => {
    await validate()
    await reqChannelEdit(formState.value)
