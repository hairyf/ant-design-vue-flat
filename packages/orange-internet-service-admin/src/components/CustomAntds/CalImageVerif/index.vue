<!--
 * @Author: Zhilong
 * @Date: 2021-05-20 09:57:37
 * @LastEditTime: 2021-05-20 18:45:48
  ...
  export default defineComponent({
    ...
    setup(props, context) {
      ...
      const sliderBaseSize = computed(() => {
        return Math.max(
          Math.min(Math.round(props.sliderSize), Math.round(canvasWidth.value * 0.5)),
          10
        )
      })
      ...
      const init = (withCanvas?: boolean) => {
        ...
        state.pinX = getRandom(
          ...
          canvasWidth.value - puzzleBaseSize.value - 10
        ) // 留10的边距
      ...
      const makeImgWithCanvas = () => {
        ...
        // 随机画10个图形
        for (let i = 0; i < 12; i++) {
          ctx.fillStyle = `rgb(${getRandom(100, 255)},${getRandom(100, 255)},${getRandom(
            100,
            255
          )})`
          ctx.strokeStyle = `rgb(${getRandom(100, 255)},${getRandom(100, 255)},${getRandom(
            100,
            255
          )})`
        ...
        const x = Math.abs(
          state.pinX -
            ...
            (puzzleBaseSize.value - sliderBaseSize.value) *
              ((styleWidth.value - sliderBaseSize.value) /
                (canvasWidth.value - sliderBaseSize.value)) -
            3
        )
        if (x < props.range) {
          // 成功
