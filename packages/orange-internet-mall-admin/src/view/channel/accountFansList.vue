 ...
 * @LastEditTime: 2021-05-10 15:14:01
 * @Description: 粉丝列表
 * @LastEditors: Mr.wang
-->
<template>
  <el-card shadow="never" class="screen-box">
    <el-row :gutter="24">
      <el-col :xl="{ span: '4_8' }" :md="8" :sm="12" class="oe-flex">
        <div class="oe-flex is-grid">
          ...
          <el-input
            ...
            placeholder="微信号/微信昵称/标签"
          ></el-input>
        </div>
      </el-col>
      <el-col :xl="{ span: '4_8' }" :md="8" :sm="12" class="oe-flex">
        <div class="oe-flex is-grid">
          <div class="screen-text">关注状态</div>
      ...
      <el-col :xl="{ span: '4_8' }" :md="8" :sm="12" class="oe-flex">
        <div class="oe-flex is-grid">
          <div class="screen-text">关注时间</div>
          <el-date-picker
      ...
      <el-col :xl="{ span: '4_8' }" :md="8" :sm="12" class="oe-flex">
        <div class="oe-flex is-grid">
          <div class="screen-text">取关时间</div>
          <el-date-picker
            prefix-icon="el-icon-date"
            class="oe-flex-1"
            v-model="formInfo.createtime"
  ...
  <el-card shadow="never" class="custom-crad-container">
    ...
    >
      ...
      <el-table-column label="粉丝" min-width="260" prop="name">
        <div class="fans-info">
          <el-image src="123"></el-image>
          <span class="name">我是用户名</span>
          <span class="blacklist">黑名单</span>
        </div>
      </el-table-column>
      <el-table-column label="微信号" min-width="200" prop="summarize"> </el-table-column>
      <el-table-column label="标签" width="130" prop="type">
        <template #default="scope">
          <el-tag>{{ scope.row.type }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="关注状态" min-width="180" prop="type">
        <template #default="scope">
          <div :class="[scope.row.type == 1 ? 'start-up' : 'disable-text']">{{
            scope.row.type == 1 ? '已关注' : '取消关注'
          }}</div>
        </template>
      </el-table-column>
      <el-table-column label="关注时间" min-width="200" prop="createtime" sortable>
        <template #default="scope">
          <span>{{ formatUnix(scope.row.createtime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="取关时间" min-width="200" prop="createtime" sortable>
        <template #default="scope">
          <span>{{ formatUnix(scope.row.createtime) }}</span>
        </template>
      </el-table-column>
    ...
    <div class="goods-list-footer">
      <div class="select-all">
        ...
        <el-button type="text" @click="selectAllChange">标签</el-button>
        <el-button type="text" @click="selectAllChange">加入黑名单 </el-button>
        <el-button type="text" @click="selectAllChange">移除黑名单</el-button>
      </div>
