 ...
 * @LastEditTime: 2021-05-11 10:40:11
 * @Description: 
 * @LastEditors: Mr.Mao
 * @autograph: 任何一个傻子都能写出让电脑能懂的代码，而只有好的程序员可以写出让人能看懂的代码
-->
<template>
  <edit-header icon="ad-commonly-used-group" title="常用图标组" />
  <edit-card-form title="组件风格">
    ...
    <el-form-item label="图标数量">
      <el-radio-group v-model="option.componentStyle.iconCount">
        <el-radio :label="v" v-for="v in ['每行3个', '每行4个', '每行5个']" :key="v" />
      </el-radio-group>
    </el-form-item>
  </edit-card-form>
  <edit-card-form title="内容编辑">
    ...
    <nested-sort-table v-model="option.contentEdit.contents">
      <template #item="{ item: v, index: i }">
        <edit-card-item
          class="oe-flex"
          @close="option.contentEdit.contents.splice(i, 1)"
          :show-close="option.contentEdit.contents.length > 1"
        >
          <div class="oe-m-r-2">
            <i class="ad-tableIcon" style="color: var(--color-primary)" />
          </div>
          <el-form label-width="50px" class="oe-flex-1">
            <el-form-item label="图片" v-if="option.componentStyle.componentModal === '图片'">
            ...
            <el-form-item label="链接">
              <link-item v-model="v.link" />
            </el-form-item>
          </el-form>
        </edit-card-item>
      </template>
    </nested-sort-table>
    <div class="inc-button" @click="onAddItem" v-if="option.contentEdit.contents.length < 20">
      + 添加
    </div>
  </edit-card-form>
...
<script lang="ts" setup>
  ...
  import { commonlyUsedGroupOption, linkOption } from '@/view/decorate/config'
  ...
  import { userLinks, commonLinks, otherLinks } from '../config/linkIndexes'
  ...
  // 基本项
  const baseItem = cloneDeep(props.option.contentEdit.contents[0])
  baseItem.text = '文本'
  baseItem.image = ''
  baseItem.link = linkOption
  const onAddItem = () => {
    const newBaseItem = cloneDeep(baseItem)
    props.option.contentEdit.contents.push(newBaseItem)
  }
</script>
