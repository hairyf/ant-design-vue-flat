 ...
 * @LastEditTime: 2021-05-11 09:22:53
 * @Description: 余额统计报表
 * @LastEditors: Mr.wang
 * @autograph: 任何一个傻子都能写出让电脑能懂的代码，而只有好的程序员可以写出让人能看懂的代码
-->
<template>
  <el-card shadow="never" class="record-box custom-card-page custom-crad-container">
    <el-row class="tips-box">
      <el-col :xl="24">
        所有会员的余额变动都会有记录，每日的统计报表会在“次日凌晨”进行更新，每月的统计报表会在“次月首日凌晨”进行更新
      </el-col>
    </el-row>
    <el-row>
    ...
    <el-table class="record-list" :data="tableData" style="width: 100%">
      <template v-slot:empty>
        <el-empty class="oe-m-t-30" description="暂无余额统计报表" />
      </template>
      <el-table-column prop="belong_date" label="日期"> </el-table-column>
      <el-table-column prop="sum_total_recharge_money" label="本期期初（元）"></el-table-column>
      ...
      <el-table-column prop="sum_money" label="本期结余（元）"></el-table-column>
  ...
  <statisticalExport v-model:show="show" :currentId="currentId" :type="'money'" :date_type="date">
  </statisticalExport>
</template>
<script setup lang="ts">
  ...
  const restRecordList = async () => {
    ...
    income_sum.value = data.data.money_sum
