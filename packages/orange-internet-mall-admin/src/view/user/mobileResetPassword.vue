 ...
 * @LastEditTime: 2021-03-27 22:13:00
 * @Description: 
...
<template>
  <div class="login-container-right-panel">
    ...
    <div class="login-container-tips verify-email"
      >验证码已发送至 <span class="text">{{ mobile }}</span></div
    >
    <el-form ref="mobileForm" :model="mobileInfo" :rules="mobileRules" class="login-container-form">
      <el-form-item>
      ...
      <el-form-item>
        <el-input type="password" placeholder="密码" v-model="mobileInfo.password"></el-input>
...
<script lang="ts" setup>
  ...
  import { reqAuthResetByMobile, reqAuthSendResetByMobile } from '@/api/user'
  import Password from '@/components/password-strength/password-strength.vue'
  import { ElMessage } from 'element-plus'
  const router = useRouter()
  const route = useRoute()
  const mobile: any = route.query.id
.log(mobile, '手机号')
  const count = ref(60)
  ...
  const next = () => {
    ...
    isResend.value = true
    resetTime(count)
    reqAuthSendResetByMobile({ mobile }).then((res) => {
      if (res.data.code != 0) {
        ElMessage({
          showClose: true,
          message: '发送失败, 请检查手机号是否正确',
          type: 'error'
        })
      }
    })
  ...
  const backLogin = () => {
    mobileForm.value.validate(async (valid: any) => {
      if (!valid) return
      await reqAuthResetByMobile(mobileInfo)
