 ...
 * @LastEditTime: 2021-05-10 17:29:07
 * @Description: 特殊日志
    ...
    <el-row :gutter="24">
      <el-col :xl="{ span: '4_8' }" :md="8" :sm="12" class="oe-flex">
        <div class="oe-flex is-grid">
          <div class="screen-text">操作类型</div>
          <el-select v-model="form.operationTypeId" placeholder="全部" class="oe-flex-1" clearable>
            <el-option
  ...
  <el-card shadow="never" class="refund-list custom-crad-container custom-card-page">
    <el-table :data="tableData" style="width: 100%">
      <template v-slot:empty>
        <el-empty class="oe-m-t-30" description="暂无特殊日志" />
      </template>
      ...
      <el-table-column label="操作事件" prop="remark"> </el-table-column>
...
<script setup lang="ts">
  ...
  import { reqSpecialList, reqRoleSelect, reqSpecialLogType } from '@/api/system'
  ...
  // 操作类型列表
  const operationTypeList = ref<any[]>([])
  reqSpecialLogType({ page: 1, limit: 100 }).then((res) => {
    operationTypeList.value = res.data.data.logo_type
  })
  // 重置按钮
  const getEmpty = () => {
    form.value.operationTypeId = ''
  ...
  const getGeneralList = async () => {
    ...
    const { data } = await reqSpecialList({
      page: pagination.currentPage,
      limit: pagination.pageSize,
      search: JSON.stringify({
        special_log_type: form.value.operationTypeId,
        createtime,
